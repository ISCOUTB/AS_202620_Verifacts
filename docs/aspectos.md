...
| A-04 | Bajo acoplamiento — un cliente HTTP nuevo (frontend) sin tocar la lógica de negocio | [Q-02](escenarios-de-calidad.md#q-02--incorporación-de-un-nuevo-analizador) | Contenedores "Interfaz web" y "Aplicación VeriFacts" — [C4 Nivel 2](c4/02-contenedores.md) | [ADR-0001](adr/0001-estilo-arquitectonico.md) | `frontend/src/api/client.ts` (cliente HTTP), `app/api/routes.py` (`GET /analysis`, `GET /analysis/{id}`), `app/persistence/repository.py` (`list_analyses`, `count_analyses`), `app/main.py` (`CORSMiddleware`) | El frontend se incorpora como cliente externo (solo HTTP/JSON) sin agregar lógica de negocio ni modificar `Content`, `Analysis` o `Scoring`; el historial pagina correctamente | `tests/test_history.py` — **3 passed**: listado paginado, detalle por `id`, `404` ante un `id` inexistente |
